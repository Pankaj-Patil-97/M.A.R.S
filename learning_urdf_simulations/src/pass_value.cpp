...
#include <sensor_msgs/JointState.h>
 ...
 double temp_pass;
//Variable to hold the data to be copied-
sensor_msgs::JointState data_sub;

//Object declarations for messages used:
sensor_msgs::JointState joint_state_publisher;

//Publisher variable-
     ros::Publisher joint_st_pub = joint_pub_nh.advertise<sensor_msgs::JointState>("/joint_states",1024);

void copy_data(const sensor_msgs::JointState::ConstPtr& msg)
{   
    data_sub.header.stamp=msg->header.stamp;
    data_sub.name=msg->name;
    data_sub.position=msg->position;
    data_sub.position=msg->velocity;
    data_sub.position=msg->effort;

    joint_state_publisher = data_sub;

    joint_st_pub.publish(joint_state_publisher);

    //joint_state_publisher.position[5]=msg->position[5];
    //temp_pass=joint_state_publisher.position[3];
}
 int main(int argc, char** argv)
 {
     ...
     //Subscriber variable-
     ros::Subscriber pass_sub = joint_pub_nh.subscribe("/hardware_states", 1024, copy_data);
     ...
     {   
         ...
         joint_state_publisher.position[3]=temp_pass;
